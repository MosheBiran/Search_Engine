...
if __name__ == '__main__':
    if os.path.isfile('results.csv') and os.access('results.csv', os.R_OK):  # TODO - CSV
        os.remove('results.csv')

    with open('results.csv', 'w', newline="") as csvfile:
        writer = csv.writer(csvfile)  # TODO - CSV
        writer.writerow(
            ["Engine Number", "Query_num", "average precision", "precision@5", "precision@10",
             "precision@50", "average recall"])
        ...
        try:
            ...
            if model_url is not None and config.get_download_model():
                ...
                if os.path.exists(dest_path):
                    ...
                    print(f'Successfully downloaded and extracted pretrained model into {model_dir}.')
                else:
                    print('model.zip file does not exists.')
                        ...
                        for i, row in queries.iterrows():
                            ...
                            if q_results_labeled is not None:
                                ...
                                if recall < 0 or recall > 1:
                                    ...
                                    # writer.writerow(["Engine Number", "Query_num", "average precision", "precision@5", "precision@10", "precision@50", "average recall"])
                                writer.writerow([engine_module, q_id, prec, p5, p10, p50, recall])  # TODO - CSV
